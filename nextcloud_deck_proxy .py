...
<div class="wrap">
  ...
  <div class="card">
    <div class="sect-row">
      <span class="sect">Localisation</span>
      <button class="btn btn-sm" onclick="loadBoards()">↻ Rafraîchir</button>
    </div>
    <div class="g2">
      <div class="f">
  ...
  <div class="card">
    ...
    <div class="f" id="tags-field" style="display:none">
      <label class="lbl">Tag</label>
      <div class="tags-wrap" id="tags-wrap"></div>
    </div>
    <div class="f" id="users-field" style="display:none">
...
function loadUsersFromBoard(){
  ...
  try{
    ...
    document.getElementById('users-field').style.display=users.length>0?'flex':'none';
  }catch(e){}
}
...
function renderTags(){
  ...
  if(!availableTags||availableTags.length===0){field.style.display='none';return;}
...
async function createCard(){
  ...
  try{
    ...
    st.className='status ok';st.textContent='Carte "'+card.title+'" créée avec succès ! Fermeture dans 3s...';btn.disabled=false;
    // Retirer le label DECK maintenant que la carte est créée
    try {
      const pf = await fetch('/get-prefill').then(r=>r.json());
      if(pf.mail_mid){
        await fetch('/remove-label',{
          method:'POST',
          headers:{'Content-Type':'application/json'},
          body:JSON.stringify({
            email: pf.mail_email,
            password: pf.mail_password,
            folder: pf.mail_folder,
            mid: pf.mail_mid,
            label: pf.mail_label
          })
        });
      }
    } catch(e){ console.log('remove-label:', e); }
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _remove_label(self):
        ...
        try:
            ...
            if folder=='SENT':
                ...
                m.select(b'"'+label.encode('ascii')+b'"')
                m.copy(mid.encode(), 'INBOX')
            m.store(mid.encode(), '+FLAGS', '\\Deleted')
            m.expunge()
            m.logout()
            result={'ok': True, 'msg': 'Label retiré'}
            print(f"  Label DECK retiré après validation.")
        except Exception as e:
            result={'ok': False, 'msg': str(e)}
            print(f"  Erreur retrait label : {e}")
    ...
    def _store_prefill(self):
        global prefill_data
        length = int(self.headers.get('Content-Length', 0))
        prefill_data = json.loads(self.rfile.read(length))
        print(f"  Prefill reçu : {prefill_data.get('titre','')[:50]}")
        b = b'OK'
        self.send_response(200)
        self.send_cors()
