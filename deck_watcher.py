...
Ta mission :
1. Choisir le tableau le plus pertinent (basé sur le nom du client/projet dans l'email). Si aucun tableau ne correspond clairement, mettre boardId à null et boardTitle à "".
...
class GmailWatcher:
    def __init__(self, cfg):
        ...
        self.processed_file = '/tmp/deck_processed.json'
    ...
    def _save_processed(self, mid):
        mid_str = mid.decode() if isinstance(mid, bytes) else str(mid)
        self.processed.add(mid_str)
        try:
            lst = list(self.processed)[-500:]
            with open(self.processed_file, 'w') as f:
                json.dump(lst, f)
        except Exception as e:
            print(f"  Erreur sauvegarde processed : {e}", flush=True)
    ...
    def fetch_deck_mails(self):
        try:
            ...
            # Recharger les mails traités depuis le fichier (mis à jour par le proxy)
            self.processed = self._load_processed()
            ...
            for mid in mail_ids:
                ...
                # Déjà traité ?
                if mid_str in self.processed or mid_str in proxy_processed:
                    print(f"  Mail {mid_str} déjà traité - ignoré", flush=True)
                    continue
