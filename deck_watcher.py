...
class GmailWatcher:
    ...
    def fetch_deck_mails(self):
        try:
            mail = self.connect()
            label_imap = self.label
            new_mails = []

            # Chercher dans le dossier DECK (mails reçus)
            status, _ = mail.select(b'"' + label_imap.encode('ascii') + b'"')
            if status == 'OK':
                ...
                for mid in mail_ids:
                    ...
                    new_mails.append(('DECK', mid, msg))
            else:
                print(f"  Label introuvable : {label_imap}")

            # Chercher aussi dans Messages envoyés avec le label DECK
            status2, _ = mail.select('"[Gmail]/Messages envoy&AOk-s"')
            if status2 == 'OK':
                _, data2 = mail.search(None, f'X-GM-LABELS "{label_imap}"')
                mail_ids2 = data2[0].split()
                for mid in mail_ids2:
                    key = b'SENT_' + mid
    ...
    def process_mail(self, folder, mid, msg):
        ...
        key = b'SENT_' + mid if folder == 'SENT' else mid
        self.processed.add(key)

    def run(self):
        print(f"\n  Deck Watcher démarré")
        print(f"  Surveillance : {self.label}")
